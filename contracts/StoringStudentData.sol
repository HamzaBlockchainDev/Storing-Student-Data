...
contract StoringStudentData {
 ...
 modifier onlyAdmin(){
        require(msg.sender == admin,"Suspicious alert, YOU'RE NOT THE ADMIN");
        _;}
        struct StudentData{
            string name;
            uint age;
       ...
       function allData() public view onlyAdmin returns(StudentData[] memory){
        return students;}
         ...
         function HighMarksOfStudent() public view returns(uint){
             uint result;
         ...
         function LowMarksOfStudent() public view returns(uint){
             uint result;
             for(uint i = 0; i < students.length; i++){
